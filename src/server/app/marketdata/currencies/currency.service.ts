...
import { ICurrencyDocument } from './currency-document.interace'
...
export class CurrencyService {
    constructor(@InjectModel('Currency') private readonly currencyModel: Model<ICurrencyDocument>) {}
    ...
    async findByCode(code: string) {
        ...
        return await this.currencyModel.findOne({ code }).exec()
    }
    ...
    async updateByCode(code: string, currency: CurrencyUpdateDto) {
        ...
        return await this.currencyModel.updateOne({ code }, currency).exec()
    }

    async deleteByCode(code: string) {
        return await this.currencyModel.deleteOne({ code }).exec()
    }

    async deleteAll() {
        return await this.currencyModel.deleteMany({}).exec()
    }
}
