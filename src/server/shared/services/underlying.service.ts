...
import { IUnderlyingDocument } from '../../app/marketdata/underlyings/underlying-document.interface'
...
export class UnderlyingService {
    constructor(@InjectModel('Underlying') private readonly underlyingModel: Model<IUnderlyingDocument>) {}

    async findAll() {
        logger.log('findAll')
        return await this.underlyingModel.find().exec()
    }

    async findByCode(code: string) {
        logger.log(`findByCode id=${code}`)
        return await this.underlyingModel.findOne({ code }).exec()
    }
    ...
    async createOne(underlying: UnderlyingCreateDto) {
        ...
        return await createdUnderlying.save()
    }
    ...
    async updateByCode(code: string, underlying: UnderlyingUpdateDto) {
        ...
        return await this.underlyingModel.updateOne({ code }, underlying).exec()
    }

    async deleteByCode(code: string) {
        return await this.underlyingModel.deleteOne({ code }).exec()
    }

    async deleteAll() {
        return await this.underlyingModel.deleteMany({}).exec()
    }
}
